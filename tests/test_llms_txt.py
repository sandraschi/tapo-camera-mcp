...
def test_tool_documentation():
    ...
    for line in nav_content.split('\n'):
        if line.startswith('- ['):
            ...
            if link_text not in ['GitHub Repository', 'Tapo API']:  # Skip external links
                assert f"#### {link_text}" in full_docs, f"Documentation for {link_text} not found in full docs"
