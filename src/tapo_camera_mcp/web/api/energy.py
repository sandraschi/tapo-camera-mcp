...
async def list_energy_devices() -> dict[str, Any]:
    ...
    try:
        ...
        for device in tapo_devices:
            ...
            if host:
                try:
                    # Get fresh data directly from Tapo API with timeout
                    ...
                    if account_email and account_password:
                        ...
                        try:
                            ...
                            client = await asyncio.wait_for(
                                ...
                                timeout=5.0  # 5 second timeout for client creation
                            )
                            device_info = await asyncio.wait_for(
                                client.get_device_info(),
                                timeout=3.0  # 3 second timeout
                            )
                            energy_usage = await asyncio.wait_for(
                                client.get_energy_usage(),
                                timeout=3.0  # 3 second timeout
                            )

                            # Get real-time current power (Tapo P115 DOES provide this!)
                            current_power_result = await asyncio.wait_for(
                            ...
                            logger.warning(f"Timeout fetching fresh data for Tapo device {device_id} at {host}, using cached data")
                            # Fall through to use cached data
                        except Exception as e:
                            logger.warning(f"Failed to fetch fresh data for Tapo device {device_id} at {host}: {e}, using cached data")
                            # Fall through to use cached data

                except Exception as e:
                    # If API call fails, use cached device data
                    logger.warning(f"Error with Tapo API for device {device_id}: {e}, using cached data")

                # Use cached device data if fresh data failed
                if current_power == 0.0 and voltage == 0.0:  # Only if we didn't get fresh data
