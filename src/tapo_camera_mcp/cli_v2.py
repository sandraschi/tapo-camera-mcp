...
class TapoCameraCLI:
    ...
    def _create_parser(self) -> argparse.ArgumentParser:
        ...
        server_parser.add_argument("--no-stdio", action="store_false", dest="stdio",
        ...
        ptz_parser.add_argument("action", choices=["move", "preset"], 
                              help="PTZ action")
        ptz_parser.add_argument("--pan", type=float, default=0.0,
                              help="Pan position (-1.0 to 1.0)")
        ptz_parser.add_argument("--tilt", type=float, default=0.0,
                              help="Tilt position (-1.0 to 1.0)")
        ptz_parser.add_argument("--speed", type=float, default=0.5,
                              help="Movement speed (0.1 to 1.0)")
        
        # Stream control
        stream_parser = camera_subparsers.add_parser("stream", help="Stream operations")
        stream_parser.add_argument("action", choices=["start", "stop"], 
                                 help="Action to perform")
        stream_parser.add_argument("--quality", choices=["hd", "sd"], default="hd",
                                 help="Stream quality")
        
        # Motion detection
        motion_parser = camera_subparsers.add_parser("motion", 
                                                    help="Motion detection control")
        motion_parser.add_argument("action", choices=["enable", "disable", "status"],
                                 help="Action to perform")
        
        # LED control
        led_parser = camera_subparsers.add_parser("led", help="LED control")
        led_parser.add_argument("action", choices=["on", "off", "status"],
                              help="Action to perform")
        
        # Privacy mode
        privacy_parser = camera_subparsers.add_parser("privacy", 
                                                     help="Privacy mode control")
        privacy_parser.add_argument("action", choices=["on", "off", "status"],
                                  help="Action to perform")
    ...
    async def _handle_connect(self, args) -> int:
        ...
        result = await self.client.call_tool("connect_camera", {
            "host": args.host,
    ...
    async def _handle_camera(self, args) -> int:
        ...
        if args.camera_command == "info":
            ...
            if args.action == "move":
                result = await self.client.call_tool("move_ptz", {
                    "pan": args.pan,
                    "tilt": args.tilt,
                    "speed": args.speed
                })
            ...
            if args.action in ["on", "off"]:
                result = await self.client.call_tool("set_led_enabled", 
            ...
            if args.action in ["on", "off"]:
                result = await self.client.call_tool("set_privacy_mode", 
                                                   {"enabled": args.action == "on"})
                if result.content.get("status") == "success":
