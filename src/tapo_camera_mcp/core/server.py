    ...
    async def _register_tools(self):
        ...
        for tool_cls in unique_tools:
            ...
            if param_names:
                ...
                # Get the docstring
                docstring = tool_cls.execute.__doc__ if hasattr(tool_cls, 'execute') and tool_cls.execute.__doc__ else 'Tool execution'

                # Create kwargs assignments
                kwargs_lines = '\n'.join([f"        kwargs['{name}'] = {name}" for name in param_names])

                # Create the function code using string formatting
                func_code = '''
async def tool_wrapper(''' + param_signature + '''):
    """''' + docstring + '''"""
    try:
...
''' + kwargs_lines + '''
        ...
        if hasattr(tool_instance, 'execute'):
            ...
            raise ValueError("Tool ''' + tool_name + ''' has no execute method")
            
    except Exception as e:
        error_msg = "Error executing tool ''' + tool_name + ''': {}".format(e)
